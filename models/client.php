...
class BillingModelClient extends JModelLegacy{
    function getCountClients($filters, $search){
        $adv = '';
        ...
        $query = "SELECT COUNT(id) FROM `#__billing_clients` WHERE ".$this->_db->QuoteName('deleted')." = ".$this->_db->Quote(0)." ".$adv;
        $this->_db->setQuery($query);
        return $this->_db->loadResult();
        //$this->_db->stderr();
    }
    function getAllClients($filters, $orderby, $order, $limitstart, $limit, $search){
        $adv = '';
        foreach($filters as $key=>$data){
            switch ($key) {
                case 'date_stop':
        ...
        $query = "SELECT * FROM `#__billing_clients` WHERE ".$this->_db->QuoteName('deleted')." = ".$this->_db->Quote(0)." ".$adv;//tyt
